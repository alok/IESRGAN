...
from models.architecture import RRDBNet, VGGFeatureExtractor, nn, torch
...
if __name__ == "__main__":
    ...
    scheduler = lr_scheduler.MultiStepLR(
        optimizer_G, [50000, 100000, 200000, 300000], 0.5
    )
    ...
    for epoch in trange(config["number_epochs"]):
        ...
        for step, (lr, hr) in enumerate(train_bar):
            ...
            writer.add_scalar(
                tag="l_g_pix", scalar_value=log_dict["l_g_pix"], global_step=global_step
            )
            writer.add_scalar(
                tag="l_g_fea", scalar_value=log_dict["l_g_fea"], global_step=global_step
            )
            ...
            if global_step % config["val_freq"] == config["val_freq"] - 1:
                netG.eval()
                out_path = (
                    config["path_to_training_results"]
                    + "/SRF_"
