...
if __name__ == "__main__":
    ...
    sr_image = np.zeros(
        (image.shape[0] * scale, image.shape[1] * scale, image.shape[2]), dtype=np.uint8
    )
    tmp = image.copy()  # for drawing a rectangle
    stepSize = default_config["crop_size"]
    (w_width, w_height) = (
        default_config["crop_size"],
        default_config["crop_size"],
    )  # window size
    for x in range(0, image.shape[0], stepSize):
        for y in range(0, image.shape[1], stepSize):
            ...
            sr_image[
                x * scale : x * scale + window.shape[0] * scale,
                y * scale : y * scale + window.shape[1] * scale,
                :,
            ] = np.moveaxis(window_sr.data.squeeze().cpu().numpy() * 255, 0, -1).astype(
                np.uint8
            )
    # show all windows
    plt.show()
