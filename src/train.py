...
from models.architecture import (
    Discriminator_VGG_224,
    Discriminator_VGG_448,
    RRDBNet,
    VGGFeatureExtractor,
    nn,
    torch,
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(
        dataset=valid_set, num_workers=2, batch_size=1, shuffle=False, drop_last=True
    )
    ...
    netG = RRDBNet(
        in_nc=3, out_nc=3, nf=16, nb=16, gc=32, upscale=default_config["upscale_factor"]
    ).to(device)
    print("# generator parameters:", sum(param.numel() for param in netG.parameters()))
    netD = (
        Discriminator_VGG_224(in_nc=3, base_nf=16).to(device)
        if config["scale"] == "x2"
        else Discriminator_VGG_448(in_nc=3, base_nf=16).to(device)
    )
    print(
    ...
    optimizer_G = torch.optim.Adam(
        optim_params, lr=config["learning_rate_G"], weight_decay=0, betas=(0.9, 0.999)
    )
    ...
    if config["warm_start"] == "True" and config["scale"] != "x4":
        checkpointD = torch.load(
            f"../output/models/netD_epoch={config['n_epoch_start']-1}"
        )
        ...
        checkpointG = torch.load(
            f"../output/models/netG_epoch={config['n_epoch_start']-1}"
        )
        ...
        checkpointD = torch.load(
            f"../output/models/netD_{config['scale']}_epoch={config['n_epoch_start']-1}"
        )
        netD.load_state_dict(checkpointD["model_state_dict"], strict=False)
        optimizer_D.load_state_dict(checkpointD["optimizer_state_dict"])

        checkpointG = torch.load(
            f"../output/models/netG_{config['scale']}_epoch={config['n_epoch_start']-1}"
        )
    ...
    for optimizer in optimizers:
        schedulers.append(
            lr_scheduler.MultiStepLR(optimizer, [50000, 100000, 200000, 300000], 0.5)
        )
    ...
    for epoch in trange(config["n_epoch_start"], config["n_epoch_end"]):
        ...
        for step, (lr, hr) in enumerate(train_bar):
            ...
            if step % config["D_update_ratio"] == 0 and step > config["D_init_iters"]:
                ...
                l_g_gan = (
                    config["loss_gan_weight"]
                    * (
                        cri_gan(pred_d_real - torch.mean(pred_g_fake), False)
                        + cri_gan(pred_g_fake - torch.mean(pred_d_real), True)
                    )
                    / 2
                )
                l_g_total += l_g_gan
            ...
            writer.add_scalar(
                tag="D_real", scalar_value=log_dict["D_real"], global_step=global_step
            )
            writer.add_scalar(
                tag="D_fake", scalar_value=log_dict["D_fake"], global_step=global_step
            )
            ...
            if global_step % config["val_freq"] == config["val_freq"] - 1:
                netG.eval()
                out_path = (
                    "../output/training_results/SRF_"
                    + str(default_config["upscale_factor"])
                    + "/"
                )
                ...
                for index, image in enumerate(val_images, 1):
                    ...
                    save_image(
                        image,
                        out_path + "global_step_%d_index_%d.png" % (global_step, index),
                        padding=5,
                    )
                # log
                writer.add_scalar(
                    tag="mse",
                    scalar_value=valing_results["mse"],
                    global_step=global_step,
                )
                writer.add_scalar(
                    tag="psnr",
                    scalar_value=valing_results["psnr"],
                    global_step=global_step,
                )
                writer.add_scalar(
                    tag="ssim",
                    scalar_value=valing_results["ssim"],
                    global_step=global_step,
                )

            # save models and training states
            if (
                global_step % config["save_checkpoint_freq"]
